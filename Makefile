...
SRCS_STATIC := $(shell find $(STATIC_DIR))
...
all: $(GEN_HTML) $(GEN_RST) $(GEN_CSS) $(GEN_JS) $(GEN_STATIC) $(OUT_DIR)/robots.txt
...
$(OUT_DIR)/articles/%.html: $(PAGES_RST_DIR)/%.rst $(TPL_DIR)/*.j2 | $(OUT_DIR) validate_python
	$(PYTHON) $(GENERATOR_DIR)/translate.py -r -b="$(TPL_DIR)/article.j2" -s="$(STATIC_DIR)" "$<" > "$@"
	$(PYTHON) $(GENERATOR_DIR)/translate.py -i -t="$(TPL_DIR)" -s="$(STATIC_DIR)" "$@"

# template each HTML file
$(OUT_DIR)/%.html: $(PAGES_DIR)/%.html $(PAGES_RST_DIR)/*.rst $(TPL_DIR)/*.j2 | $(OUT_DIR) validate_python
	$(PYTHON) $(GENERATOR_DIR)/translate.py -t="$(TPL_DIR)" -s="$(STATIC_DIR)" "$<" > "$@"
...
# directories
$(OUT_DIR): $(OUT_DIR)/articles $(OUT_DIR)/css $(OUT_DIR)/js $(OUT_DIR)/static
